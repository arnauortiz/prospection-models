...
def statistics(dataset, output_path):
    ...
    print(f"   Desviación estándar: {std_score:.3f}")

    print("\n📈 Estadísticas de Similitud:")
    print(f"   Mediana: {med_sim:.3f}")
    print(f"   Media: {mean_sim:.3f}")
    print(f"   Desviación estándar: {std_sim:.3f}")
    ...
    axs[0].set_title(
        f"Score per paraula clau | Mediana: {med_score:.3f} | Mitjana: {mean_score:.3f} | ±1σ: {std_score:.3f}"
    )
    ...
    axs[1].set_title(
        f"Similitud coseno | Mediana: {med_sim:.3f} | Mitjana: {mean_sim:.3f} | ±1σ: {std_sim:.3f}"
    )
...
def tsne_visualization_fp(combined_df, output_path):
    print("\n" + "=" * 50)
    print("🔍 VISUALIZACIÓN T-SNE CON FALSOS POSITIVOS")
    print("=" * 50)
    ...
    plt.title(
        "Visualització 2D dels embeddings per sector (amb falsos positius marcats)"
    )
    plt.xlabel("")
    plt.ylabel("")
